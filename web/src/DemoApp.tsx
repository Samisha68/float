...
export default function DemoApp({
  ...
  providedLedger?: Ledger;
}) {
  const fallback = useMemo(() => new DemoLedger(), []);
  const ledger = providedLedger || fallback;
  ...
  return (
    <div className="min-h-full flex flex-col">
      ...
      <div className="px-8 py-3 text-sm">
        ...
        {ledger.mode === "demo"
          ? "Sample data. No transactions are sent."
          : "Devnet test tokens only."}
        {busy && <span role="status"> · Transaction in progress…</span>}
        {signature && (
          <a
      ...
      <nav className="border-b" style={{ borderColor: "var(--color-border)" }}>
        <div className="mx-auto max-w-6xl px-8 flex gap-8 overflow-x-auto">
      ...
      {notice && (
        <div className="mx-auto max-w-6xl w-full px-8 pt-6">
          <div
            className="settle rounded-[10px] px-4 py-3 text-sm border"
            style={{
      ...
      <main className="mx-auto max-w-6xl w-full px-8 py-10 flex-1">
        <fieldset disabled={busy}>
              ...
              onRepay={async (id) =>
                run(async () => {
                  try {
                    ...
                    if (ledger.mode === "devnet")
                      setSignature(a.signature || null);
                    setNotice(
                      `Repaid ${usd(a.totalDue)}. Recorded against your credit history.`,
                    );
                    await refresh();
                  } catch (e) {
                    setNotice(chainError(e));
                  }
                })
              }
            />
                ...
                run(async () => {
                  try {
                    ...
                    if (ledger.mode === "devnet")
                      setSignature(a.signature || null);
                    setNotice(
                      `Approved. ${usd(a.amount)} USDC disbursed — repayment of ${usd(a.totalDue)} due ${shortDate(a.dueAt!)}.`,
                    );
                    await refresh();
                  } catch (e) {
                    setNotice(chainError(e));
          ...
          {tab === "record" && business && (
            <RecordScreen business={business} advances={advances} />
          )}
        </fieldset>
      </main>
      ...
      >
        <div
          className="mx-auto max-w-6xl px-8 text-xs"
          style={{ color: "var(--color-grey)" }}
        >
          Float · {ledger.mode === "demo" ? "Offline demo" : "Solana devnet"} ·
          tier 1 ceiling{" "}
          <span className="tabular">{usd(POLICY.MAX_ADVANCE_TIER_1)}</span>,
          term{" "}
          <span className="tabular">
            {POLICY.MIN_TERM_DAYS}–{POLICY.MAX_TERM_DAYS}
          </span>{" "}
          days.
        </div>
      </footer>
...
function Header({ mode }: { mode: "demo" | "devnet" }) {
  return (
    <header style={{ background: "var(--color-ink)" }} className="text-white">
      <div className="mx-auto max-w-6xl px-8 py-5 flex items-baseline justify-between">
        <div className="flex items-baseline gap-3">
          <img src="/float-favicon.svg" alt="Float" className="h-10 w-12" />
...
function TabButton({
  ...
  return (
    <button
      ...
      style={{
        color: active ? "var(--color-ink)" : "var(--color-grey)",
        fontWeight: active ? 600 : 400,
        boxShadow: active ? "inset 0 -2px 0 var(--color-ink)" : "none",
      }}
    >
      {children}
    </button>
...
function Field({
  ...
  return (
    <label className="block">
      <span className="block text-sm mb-2" style={{ fontWeight: 500 }}>
        {label}
      </span>
      {children}
      {hint && (
        ...
        {overdueSoon && advance.dueAt !== null && (
          <>
            ...
            <strong style={{ color: "var(--color-ink)" }}>
              Due in {daysUntil(advance.dueAt)} days.
            </strong>
          </>
        )}
      </div>
...
function ApplyScreen({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-16 settle">
      <div>
        <h1 className="display text-3xl mb-2">Request an advance</h1>
        <p className="text-sm mb-8" style={{ color: "var(--color-grey)" }}>
          Against a payment already owed to you. Independent verification is
          part of the planned financing process.
        </p>
        <form onSubmit={submit} className="grid gap-6 max-w-lg">
          <Field
            label="Who owes you the money?"
            hint="Enter a sample customer name for this test."
          >
            <input
              style={inputStyle}
              value={payer}
          ...
          <Field
            label="Amount they owe (USDC)"
            hint="Your advance cannot exceed this figure."
          >
            <input
              style={inputStyle}
              className="tabular"
          ...
          >
            <input
              ...
              min={1}
              max={POLICY.MAX_ADVANCE_TIER_1}
              required
            />
          </Field>
          <Field
            label="Term (days)"
            hint={`${POLICY.MIN_TERM_DAYS}–${POLICY.MAX_TERM_DAYS} days.`}
          >
            <input
              style={inputStyle}
              className="tabular"
              ...
              min={POLICY.MIN_TERM_DAYS}
              max={POLICY.MAX_TERM_DAYS}
              required
            />
          </Field>
          <div>
      ...
      <aside
        className="text-sm border-l pl-8"
        style={{ borderColor: "var(--color-divider)" }}
      >
        ...
        <ol className="grid gap-4" style={{ color: "var(--color-grey)" }}>
          <li>
            ...
            Verified against the register, independently of you.
          </li>
          <li>
            <strong style={{ color: "var(--color-ink)" }}>
              2. The obligation exists.
            </strong>{" "}
            Confirmed with the payer, on a channel we source.
          </li>
          <li>
            <strong style={{ color: "var(--color-ink)" }}>
              3. It is not financed twice.
            </strong>{" "}
            Checked against every invoice we have seen.
          </li>
          <li>
            <strong style={{ color: "var(--color-ink)" }}>
              4. The advance fits the inflow.
            </strong>{" "}
    ...
    <div className="settle">
      ...
      <div className="grid gap-4">
        {advances.map((a) => (
          <article
            key={a.id}
            className="border p-6 grid grid-cols-1 md:grid-cols-[1fr_200px_200px] gap-8 items-center"
            style={{
              borderColor: "var(--color-border)",
          ...
          >
            ...
            <div className="text-right">
              {a.dueAt && (
                <div
                  className="text-xs tabular mb-3"
                  style={{ color: "var(--color-grey)" }}
                >
              ...
              {(a.status === "Active" || a.status === "Overdue") && (
                <PrimaryButton onClick={() => onRepay(a.id)}>
                  Repay {usd(a.totalDue)}
                </PrimaryButton>
              )}
            </div>
...
function UnderwritingScreen({
  ...
  advances: Advance[];
  onApprove: (id: string, feeBps: number) => void;
}) {
  ...
  return (
    <div className="settle">
      ...
      <p className="text-sm mb-8" style={{ color: "var(--color-grey)" }}>
        Approval disburses USDC immediately. The contract rejects anything above
        the tier-1 ceiling or above the verified inflow, whatever is entered
        here.
      </p>
      {queue.length === 0 ? (
        ...
        <div className="grid gap-4">
          {queue.map((a) => (
            <QueueRow key={a.id} advance={a} onApprove={onApprove} />
          ))}
        </div>
      )}
    </div>
...
function QueueRow({
  ...
  const coverage =
    advance.expectedInflow > 0 ? advance.amount / advance.expectedInflow : 0;
  return (
    <article
    ...
    >
      <div className="grid grid-cols-[1fr_auto] gap-8 items-start">
        <div>
          ...
          <div className="text-lg mb-4" style={{ fontWeight: 600 }}>
          ...
          <div className="text-sm mt-4" style={{ color: "var(--color-grey)" }}>
            Payer:{" "}
            <span style={{ color: "var(--color-ink)" }}>{advance.payer}</span>
          </div>
        </div>
  ...
  return (
    <div className="settle">
      ...
      <p
        ...
        style={{ color: "var(--color-grey)" }}
      >
        Written by the contract on every repayment, and readable by anyone. It
        moves your price and your speed. It does not raise your ceiling — that
        is bounded by verified inflow, so no repayment history can inflate it.
      </p>
      <div
      ...
      >
        <div className="text-xl mb-6" style={{ fontWeight: 600 }}>
          {business.legalName}
        </div>
        <dl className="grid grid-cols-4 gap-8">
          <Stat
            ...
            value={String(business.advancesTaken)}
          />
          <Stat
            large
            label="Repaid in full"
            value={String(business.advancesRepaid)}
          />
          <Stat
            large
            label="Ever past due"
            value={String(business.advancesOverdue)}
          />
          <Stat
            large
            label="Volume repaid"
            value={usd(business.totalVolumeRepaid)}
          />
        </dl>
        <div
          className="text-xs mt-6 tabular"
          style={{ color: "var(--color-grey)" }}
        >
...
function Stat({
  ...
  return (
    <div>
      ...
      <dd
        className={`tabular ${large ? "text-2xl display" : ""}`}
        style={{ fontWeight: large ? 600 : 500 }}
      >
        {value}
      </dd>
    </div>
